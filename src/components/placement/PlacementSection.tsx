...
const SkeletonCard: React.FC = () => (
  <div className={styles.card} aria-busy="true">
    ...
    <div className={styles.skeletonTags}>
      {[1,2,3].map(i => (
        <div key={i} className={`${styles.skeleton} ${styles.skeletonTag}`} />
      ))}
    </div>
...
const JobCard: React.FC<{ job: JSearchJob }> = ({ job }) => {
  ...
  const isNew = (() => {
    ...
    return Date.now() - posted.getTime() < 24 * 60 * 60 * 1000; // < 24 h
  })();
...
const PlacementSection: React.FC = () => {
  ...
  // Trigger search on Enter or preset click
  const handleSearch = () => {
    if (searchInput.trim()) {
      setActiveQuery(searchInput.trim() + ' India');
    }
  };

  const { jobs, loading, error, refetch } = useJSearchJobs({
    query: activeQuery,
    numPages: 2,
    country: 'in',
  });
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.pageHeader}>
        ...
        <button className={styles.liveIndicator} onClick={refetch} title="Refresh jobs">
      ...
      {!error && !loading && filtered.length === 0 && (
        <div className={styles.emptyState}>
          ...
          <p>No jobs found for this filter. Try another category.</p>
        </div>
      )}
    </div>
  );
